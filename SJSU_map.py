    ...
    if building_names == "Aggregated campus":
        ...
        for column in dfs.columns:  # Assuming all DataFrames have the same columns
            ...
            if 'cs heating' in column.lower():
                color = 'rgba(255, 0, 0, 1)'  # Semi-transparent red for heating
                building_data = -building_data  # Mirror image for heating
                fill = 'tozeroy'  # Fill to zero


            # Assign colors based on column names
            elif 'other' in column.lower():
                continue
...
def main():
    ...
    if df_CS2:
        ...
        st.write(1,final_df_CS["Total Heating Loads"],round(final_df_CS["Total Heating Loads"].sum()))
    ...
    if selected_aggregated_loads:
        if df_CS2:
            aggregate_selected_df = final_df_CS[["CS Electricity Loads(kBtu)","CS Heating Loads(kBtu)","CS Cooling Loads(kBtu)","CS DHW Loads(kBtu)","Other Heating Process(kBtu)","Space Heating + DHW","Total Heating Loads","Simultaneuos Loads(H)","Simultaneuos Loads(C)"]]


            
            aggregate_monthly = pd.DataFrame()

            aggregate_monthly = final_df_CS[["CS Electricity Loads(kBtu)","CS Heating Loads(kBtu)","CS Cooling Loads(kBtu)","CS DHW Loads(kBtu)","Other Heating Process(kBtu)","Space Heating + DHW","Total Heating Loads","Simultaneuos Loads(H)","Simultaneuos Loads(C)"]]
