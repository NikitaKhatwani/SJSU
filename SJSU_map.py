...
def main():
    ...
    if df_CS2:
        ...
        percent_sim_total = (total_simult_load/total_hc_load)*100
        final_df_CS["Peak Simultaneuos Loads"] =pd.NA
        final_df_CS.at[0,"Peak Simultaneuos Loads"] = max_simult_load
        final_df_CS["Total Simultaneuos Loads"] =pd.NA
        final_df_CS.at[0,"Total Simultaneuos Loads"] = total_simult_load
        final_df_CS["Total Heating Loads"] =pd.NA
        final_df_CS.at[0,"Total Heating Loads"] = total_heating_load
        final_df_CS.at[1,"Total Heating Loads"] = f"{round(percent_sim_heating, 1)}%"
        final_df_CS["Total Cooling Loads"] =pd.NA
        final_df_CS.at[0,"Total Cooling Loads"] = total_cooling_load
        final_df_CS.at[1,"Total Cooling Loads"] = f"{round(percent_sim_cooling, 1)}%"
        final_df_CS["Total H&C Loads"] =pd.NA
        final_df_CS.at[0,"Total H&C Loads"] = total_hc_load
        final_df_CS.at[1,"Total H&C Loads"] = f"{round(percent_sim_total, 1)}%"




        # Show the result
        st.write("**Simultaneous Loads & Totals:**")
        ...
        if 'saved_df' not in st.session_state:
            st.session_state.saved_df = pd.DataFrame(columns=[
                "Building Names",
                "Peak Simultaneuos Loads",
                "Total Simultaneuos Loads",
                "Total Heating Loads",
                "Total Cooling Loads",
                "Total H&C Loads"
            ])
        ...
        if saveGroup:
             ...
             data_to_append = final_df_CS[["Peak Simultaneuos Loads","Total Simultaneuos Loads","Total Heating Loads", "Total Cooling Loads", "Total H&C Loads"]]
